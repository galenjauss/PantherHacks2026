<script lang="ts">
	...
	// ── Cut ranges for timeline ───────────────────────────────────────
	const TYPE_MAP: Record<string, "filler" | "pause" | "restart"> = {
		filler: "filler",
		pause: "pause",
		restart: "restart",
		mouth: "filler",
	...
	const cutRanges = $derived(
		clips.map((c) => {
			const [m, s] = c.time.split(":").map(Number);
			const start = m * 60 + s;
			const dur = parseFloat(c.dur);
			return { start, end: start + dur, type: TYPE_MAP[c.type] };
		}),
	);
...
<div class="flex h-full">
	...
	>
		...
		<!-- Waveform / timeline -->
		<div
			class="border-t border-[#222222] bg-[#111111] px-4 py-3 flex-shrink-0 pb-[120px] overflow-hidden"
		>
			<ScrubbableTimeline duration={120} {cutRanges} />
		</div>
	</main>
</div>
